...
function checkAnswerDivision(num1, num2) {
  ...
  for (let i = 0; i < quanAnswerDivision; i++) {
    if (
      document.getElementById('inp_division_line_' + 0 + '_' + i).value !=
      undefined
    ) {
      arrInp[0].push(
        document.getElementById('inp_division_line_' + 0 + '_' + i).value
  ...
  for (let i = 1; i <= numOfCalDivision; i++) {
    ...
    for (let j = 0; j < quanNum1; j++) {
      if (
        document.getElementById('inp_division_line_' + i + '_' + j).value !=
        undefined
      ) {
        arrInp[i].push(
          document.getElementById('inp_division_line_' + i + '_' + j).value
  ...
  if (JSON.stringify(arrInp[0]) != JSON.stringify(arrAnwer[0])) return 0;
  ...
  if (quanNum2 > 1) {
    for (let i = 1; i <= quanNum2 + 1; i++) {
      ...
      for (let j = quanResultMulti; j >= 1; j--) {
        ...
        arrTichRiengInp[i].push(
          document.getElementById('inp_line_' + i + '_' + j).value
        );
      }
    }
  } else {
...
function calculationOperatorMultiply(num1, num2, operator) {
  ...
  document.addEventListener('keydown', function (event) {
    ...
    if (thisEventId.length >= 12) {
      // Đảm bảo rằng chuỗi ID có ít nhất 12 ký tự
      var newEventId;
      var newElement;
...
function calculationOperatorDivision(num1, num2) {
  ...
  document.addEventListener('keydown', function (event) {
    ...
    if (thisEventId.length >= 18) {
      // Đảm bảo rằng chuỗi ID có ít nhất 18 ký tự
      var newEventId;
      var newElement;
  ...
  while (true) {
    ...
    if (operator == '*') {
      ...
      if (operator == '/') {
        ...
        if (quanNum(num1) != num1Digits || num1 == num2) continue;
  ...
  if (socau > parseInt(num3Digits.split('/')[1])) {
    alert(
      'Kết thúc bài kiểm tra\nBạn được: ' +
        ...
        '/' +
        parseInt(num3Digits.split('/')[1]) +
        ' điểm!'
    );
    closeKiemtra();
  } else {
    document.getElementById('socau').innerText =
      socau + '/' + num3Digits.split('/')[1];
    generateCalculation();
...
function start() {
  ...
  if (operator == '/' && num1Digits < num2Digits) {
    alert(
      'Phép chia bạn cài đặt không hợp lệ\nVui lòng chọn số chữ số của số thứ nhất lớn hơn hoặc bằng số chữ số của số thứ hai'
    );
    return;
  }
